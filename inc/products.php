...
function get_products_recent() {
    ...
    try {
        //sql query to retrive only 4 rows in descending order - 4 products - the most recent. 
        $results = $db->query("
...
function get_products_search($s) {
    ...
    try {
        $results = $db->prepare("
            ...
            WHERE name LIKE ?
            ORDER BY sku");
        $results->bindValue(1, "%" . $s . "%"); // value in variable AT THE MOMENT of the binding.
        $results->execute(); //execute the query
    } catch (Exception $e) {
        echo "Data could not be retrieved from the database.";
        exit;
    }

    $results = $results->fetchAll(PDO::FETCH_ASSOC); //place the results into an associative array. 
...
function get_products_subset($positionStart, $positionEnd) {
    ...
    try {
        // will be using placeholders so will need to bind to params here and sanitize
        $results = $db->prepare("
            SELECT name, price, img, sku, paypal
            FROM products
            ORDER BY sku
            LIMIT ?, ?"); //start at 0 and return 8 shirts
    ...
    $subset = $results->fetchAll(PDO::FETCH_ASSOC); //place results into an associative array
